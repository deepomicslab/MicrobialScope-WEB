...
const AnalysisLinearVirulenceFactorsTrack = ({
    xScale,
    yCenter,
    arrowHeight,
    virulenceFactors,
    ...
    const processedVirulenceFactors = useMemo(
        () => extractAnalysisVirulenceFactorData(virulenceFactors),
        [virulenceFactors]
    );
    ...
    const showTooltip = (event, vf) => {
        toolTipRef?.current?.showTooltip(event, AnalysisVirulenceFactorTooltipTemplate(vf));
    };
    const hideTooltip = () => {
...
export default AnalysisLinearVirulenceFactorsTrack;
