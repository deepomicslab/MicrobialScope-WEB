...
import { AntibioticResistanceTooltipTemplate } from "@/components/Visualization/tooltip/AntibioticResistanceTooltipTemplate";

const LinearAntibioticResistanceTrack = ({
    xScale,
    yCenter,
    arrowHeight,
    ...
    const processedAntibioticResistance = useMemo(
        () => extractAntibioticResistanceArrowData(antibioticResistance),
        [antibioticResistance]
    );
    ...
    const showTooltip = (event, item) => {
        toolTipRef?.current?.showTooltip(
            event,
            AntibioticResistanceTooltipTemplate(item)
        );
    };
    const hideTooltip = () => {
...
export default LinearAntibioticResistanceTrack;
