...
import {
    buildVirulenceFactorsTableColumns
...
const GenomeVirulenceFactorsDetail = ({ fastaDetail, proteins }) => {
    ...
    const {
        ...
        isLoading,
        error
    } = useSWR(`${GENOMEDETAILCONFIG[microbe][magStatus]['genomeVirulenceFactorsURL']}?genomeId=${genomeId}`, fetcher)
    ...
    const columns = buildVirulenceFactorsTableColumns(handleDetailClick)
    const filteredVirulenceFactors = useMemo(() => {
        if (!virulenceFactors || !fastaDetail['contig']) return []
        return virulenceFactors.filter(p => p['contig_id'] === fastaDetail['contig'])
    }, [fastaDetail, virulenceFactors])
    ...
    return (
        <>
            {
                filteredVirulenceFactors.length > 0 ? (
                    <>
                        <Stack spacing={2}>
                            <H6
                                sx={{
                            ...
                            <Stack>
                                ...
                                <StyledTable
                                    ...
                                    dataSource={filteredVirulenceFactors}
                                    scroll={{ y: 55 * 12 }}
                                />
                            </Stack>
                            <Stack spacing={2}>
...
export default GenomeVirulenceFactorsDetail
