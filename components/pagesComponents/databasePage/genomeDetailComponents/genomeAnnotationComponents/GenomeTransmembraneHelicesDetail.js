...
import {
    buildTransmembraneHelicesTableColumns
...
import {
    TransmembraneHelicesModalDetailDescriptions, TransmembraneHelicesModalDetailTitle
} from "@/components/pagesComponents/databasePage/dataModalDetailComponents/TransmembraneHelicesModalDetailComponents"
import AnnotatedTransmembraneHelicesMapViz
    from "@/components/pagesComponents/databasePage/genomeDetailComponents/genomeAnnotationVizComponents/AnnotatedTransmembraneHelicesMapViz"
...
const GenomeTransmembraneHelicesDetail = ({ fastaDetail, proteins }) => {
    const { microbe, magStatus, genomeId } = useDatabaseGenomeDetailContext()

    const {
        data: transmembraneHelices,
        isLoading,
        error
    } = useSWR(`${GENOMEDETAILCONFIG[microbe][magStatus]['genomeTransmembraneHelicesURL']}?genomeId=${genomeId}`, fetcher)
    ...
    const columns = buildTransmembraneHelicesTableColumns(handleDetailClick)
    const filteredTransmembraneHelices = useMemo(() => {
        if (!transmembraneHelices || !fastaDetail['contig']) return []
        return transmembraneHelices.filter(p => p['contig_id'] === fastaDetail['contig'])
    }, [fastaDetail, transmembraneHelices])
    ...
    return (
        <>
            {
                filteredTransmembraneHelices.length > 0 ? (
                    <>
                        <Stack spacing={2}>
                            <H6
                                sx={{
                            ...
                            <Stack>
                                ...
                                <StyledTable
                                    ...
                                    dataSource={filteredTransmembraneHelices}
                                    scroll={{ y: 55 * 12 }}
                                />
                            </Stack>
                            <Stack spacing={2}>
                                ...
                                >
                                    <AnnotatedTransmembraneHelicesMapViz
                                        ...
                                        transmembraneHelices={filteredTransmembraneHelices}
...
export default GenomeTransmembraneHelicesDetail
