...
const AnnotatedAntibioticResistanceMapViz = forwardRef(({ fastaDetail, proteins, antibioticResistance, mode }, ref) => {
    const { width } = useContainerSize()

    const svgRef = useRef(null)
    const toolTipRef = useRef(null)

   const model = useGenomeMapVizModel('annotatedAntibioticResistance', mode, fastaDetail, proteins, width, antibioticResistance)
    ...
    const renderers = {
        circular: CircularAnnotatedAntibioticResistanceMap,
        linear: LinearAnnotatedAntibioticResistanceMap
    }
...
const CircularAnnotatedAntibioticResistanceMap = ({ model, toolTipRef }) => {
    const {
        config,
        svgWidth,
        domain,
        ...
        contigName,
        proteins,
        entities: antibioticResistanceGene,
        layout
    } = model
    const {
        cx,
    ...
    return (
        <>
            ...
            />
            <AntibioticResistanceArc
                cx={cx}
                cy={cy}
                radicalScale={angleScale}
                radius={vizConfig.antibioticResistanceGene.radius}
                antibioticResistance={antibioticResistanceGene}
                arrowWidth={vizConfig.antibioticResistanceGene.arrowWidth}
                toolTipRef={toolTipRef}
            />
            <AreaPlot
                width={vizConfig.areaPlot.width}
...
const LinearAnnotatedAntibioticResistanceMap = ({ model, toolTipRef }) => {
    const {
        svgWidth,
        domain,
        domainEnd,
        ...
        <>
            ...
            <g transform={`translate(${xOffset}, 0)`}>
                ...
                <LinearProteinTrack
                    ...
                    toolTipRef={toolTipRef}
                />
                <LinearAntibioticResistanceTrack
                       xScale={linearScale}
...
export default AnnotatedAntibioticResistanceMapViz
