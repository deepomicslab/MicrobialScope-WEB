...
const AnnotatedProteinMapViz = forwardRef(({ fastaDetail, proteins, mode = 'circular' }, ref) => {
    const { width } = useContainerSize()

    const svgRef = useRef(null)
    const toolTipRef = useRef(null)

    const model = useGenomeMapVizModel('protein', mode, fastaDetail, proteins, width)
    ...
    const renderers = {
        circular: CircularProteinMap,
        linear: LinearProteinMap
    }
...
const CircularProteinMap = ({ model, toolTipRef }) => {
    const {
        config,
        svgWidth,
        domain,
    ...
    return (
        <>
            ...
            />
            {
                domain[1] - domain[0] < 25000 ? (
                    <ProteinProductLabelArc
                        cx={cx}
                        cy={cy}
                        radicalScale={angleScale}
                        radius={vizConfig.productLabel.radius}
                        proteinRadius={vizConfig.protein.radius}
                        proteins={proteins}
                        toolTipRef={toolTipRef}
                    />
                ) : (
...
const LinearProteinMap = ({ model, toolTipRef }) => {
    const {
        svgWidth,
        domain,
        domainEnd,
