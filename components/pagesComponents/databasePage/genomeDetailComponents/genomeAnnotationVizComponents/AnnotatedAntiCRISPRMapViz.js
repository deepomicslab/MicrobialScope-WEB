...
import AntiCRISPRArc from "@/components/Visualization/vizD3/circoMapViz/AntiCRISPRArc"
...
import LinearAntiCRISPRTrack from "@/components/Visualization/vizD3/linearMapViz/AntiCRISPRLinearTrack"

const AnnotatedAntiCRISPRMapViz = forwardRef(({ fastaDetail, proteins, antiCRISPR, mode='circular' }, ref) => {
    const { width } = useContainerSize()

    const svgRef = useRef(null)
    const toolTipRef = useRef(null)

    const model = useGenomeMapVizModel('antiCRISPR', mode, fastaDetail, proteins, width, antiCRISPR)
    ...
    const renderers = {
        circular: CircularAntiCRISPRMap,
        linear: LinearAntiCRISPRMap
    }
...
AnnotatedAntiCRISPRMapViz.displayName = "AnnotatedAntiCRISPRMapViz"

const CircularAntiCRISPRMap = ({ model, toolTipRef }) => {
    const {
        config,
        svgWidth,
        domain,
    ...
    return (
        <>
            ...
            <AntiCRISPRArc
                ...
                cy={cy}
                radicalScale={angleScale}
                radius={vizConfig.antiCRISPR.radius}
                antiCRISPR={antiCRISPR}
                arrowWidth={vizConfig.antiCRISPR.arrowWidth}
                toolTipRef={toolTipRef}
            />
            <AreaPlot
                width={vizConfig.areaPlot.width}
...
const LinearAntiCRISPRMap = ({ model, toolTipRef }) => {
    const {
        svgWidth,
        domain,
        domainEnd,
        ...
        <>
            ...
            <g transform={`translate(${xOffset}, 0)`}>
                ...
                <LinearProteinTrack
                    ...
                    toolTipRef={toolTipRef}
                />
                <LinearAntiCRISPRTrack
                    xScale={linearScale}
                    yCenter={antiCRISPRYBase}
                    arrowHeight={vizConfig.protein.arrowWidth}
                    antiCRISPR={antiCRISPR}
                    toolTipRef={toolTipRef}
                />
...
export default AnnotatedAntiCRISPRMapViz
