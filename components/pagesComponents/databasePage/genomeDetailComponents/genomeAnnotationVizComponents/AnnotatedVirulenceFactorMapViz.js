...
const AnnotatedVirulenceFactorMapViz = forwardRef(({ fastaDetail, proteins, virulenceFactors, mode }, ref) => {
    const { width } = useContainerSize()

    const svgRef = useRef(null)
    const toolTipRef = useRef(null)

    const model = useGenomeMapVizModel('virulenceFactor', mode, fastaDetail, proteins, width, virulenceFactors)
    ...
    const renderers = {
        circular: CircularVirulenceFactorMap,
        linear: LinearVirulenceFactorMap
    }
...
const CircularVirulenceFactorMap = ({ model, toolTipRef }) => {
    const {
        config,
        svgWidth,
        domain,
    ...
    return (
        <>
            ...
            <VirulenceFactorsArc
                ...
                cy={cy}
                radicalScale={angleScale}
                radius={vizConfig.virulenceFactor.radius}
                virulenceFactors={virulenceFactors}
                arrowWidth={vizConfig.virulenceFactor.arrowWidth}
                toolTipRef={toolTipRef}
            />
            <AreaPlot
                width={vizConfig.areaPlot.width}
...
const LinearVirulenceFactorMap = ({ model, toolTipRef }) => {
    const {
        svgWidth,
        domain,
        domainEnd,
...
export default AnnotatedVirulenceFactorMapViz
