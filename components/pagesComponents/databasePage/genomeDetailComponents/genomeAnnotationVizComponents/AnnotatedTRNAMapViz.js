...
const AnnotatedTRNAMapViz = forwardRef(({ fastaDetail, proteins, tRNAs, mode='circular' }, ref) => {
    const { width } = useContainerSize()

    const svgRef = useRef(null)
    const toolTipRef = useRef(null)

    const model = useGenomeMapVizModel('tRNA', mode, fastaDetail, proteins, width, tRNAs)
    ...
    const renderers = {
        circular: CircularTRNAMap,
        linear: LinearTRNAMap
    }
...
const CircularTRNAMap = ({ model, toolTipRef }) => {
    const {
        config,
        svgWidth,
        domain,
    ...
    return (
        <>
            ...
            <TRNAArc
                ...
                cy={cy}
                radicalScale={angleScale}
                radius={vizConfig.tRNA.radius}
                tRNAs={tRNAs}
                arrowWidth={vizConfig.tRNA.arrowWidth}
                toolTipRef={toolTipRef}
            />
            <AreaPlot
                width={vizConfig.areaPlot.width}
...
const LinearTRNAMap = ({ model, toolTipRef }) => {
    const {
        svgWidth,
        domain,
        domainEnd,
...
export default AnnotatedTRNAMapViz
